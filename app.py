...
with tab2:
    ...
    if st.button("🚀 Verileri Ayrıştır ve Üzerine Ekle", use_container_width=True):
        if raw_text:
            ...
            for line in lines:
                line = line.strip()
                # Başlıkları, emojili çizgileri ve alt bilgi yazılarını pas geç
                ...
                if "İZİNLİ" in line.upper():
                    status = "İZİNLİ"
                    # Kelimeleri ayır, ilk kelimeyi isim al
                    ...
                    # Satırdaki özel işaretleri boşluğa çevirerek temizle (SyntaxError riskine karşı güvenli temizlik)
                    clean_line = line.replace("➔", " ").replace("»", " ").replace(":", " ").replace("-", " ").replace(",", " ")
                    # Sütun kalıntı kelimelerini sil
                    ...
                    if nums:
                        val = nums[0] # Listelerdeki ana miktar sayısı daima ilk sayıdır
                    else:
                        continue
                    ...
                    if existing:
                        cursor.execute("""
                            UPDATE weekly_data SET 
                                terfi = terfi + ?, egitim = egitim + ?, maas = maas + ?, destek = destek + ?, rozet = rozet + ?, status = ?
                            WHERE username=? AND week_start=?
                        """, (t_inc, e_inc, m_inc, d_inc, r_inc, status, username, selected_week_start))
                    else:
                        cursor.execute("""
                            INSERT INTO weekly_data (username, week_start, terfi, egitim, maas, destek, rozet, status)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (username, selected_week_start, t_inc, e_inc, m_inc, d_inc, r_inc, status))
                        
                    count += 1
                    
            conn.commit()
            st.success(f"Başarılı! Listeden {count} kişi ayıklandı ve verileri '{bulk_category}' hanesine toplandı.")
            st.rerun()
